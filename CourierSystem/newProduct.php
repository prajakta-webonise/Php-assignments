		...
		if(!isset($message))
		{
			...
			if($order_id)
			{
				...
				$body = '<h4>Welcome to Courier System</h4>
					<h5>Dear,'.$_COOKIE['user_name'].'</h5>
					<p>Following are the details of the order you placed</p>
					<p>Product Name:'.$userData['productName'].'</p>
					<p>Product Weight:'.$userData['productWeight'].'</p>
					<p>To:'.$userData['to'].'</p>
					<p>Address Line1:'.$userData['line1'].'</p>
					<p>Address Line2:'.$userData['line2'].'</p>
		...
		<div class="form-group">
  		...
  		<?php
  			...
  			foreach($opt as $value){
  				echo '<option value="'.$value['id'].'">'.$value['from_range'].'&nbsp;-&nbsp;'.$value['to_range'].'(Rs.'.$value['price'].')</option>';
  		}
